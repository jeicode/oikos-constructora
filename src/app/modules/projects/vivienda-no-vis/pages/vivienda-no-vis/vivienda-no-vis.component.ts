...
@Component({
  ...
  templateUrl: './vivienda-no-vis.component.html',
  styleUrls: ['./vivienda-no-vis.component.css']
})
export class ViviendaNoVisComponent implements OnInit {
  ...
  async getData() {
    this.data = await this.pageService.getContentPage('proyectos-construccion-vivienda-no-vis')
  }
  ...
  async getImagenes() {
    this.imagenes = await this.pageService.getImagesBySlugPage('proyectos-construccion-vivienda-no-vis');
  ...
  async getProyectos() {
    ...
    this.proyectos = this.filtrarVis(this.proyectos);
  ...
  filtrarVis(proyectos: any){
    let temp = [];
    for(let i in proyectos){
      if(proyectos[i]?.vis=='no'){
        temp.push(proyectos[i])
      }
    }
